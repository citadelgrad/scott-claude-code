...
`ownership-history-event-v1` is append-only canonical JSONL with issue/run/epoch/status/token hash, previous-event SHA-256, operation ID, and timestamp. While holding a POSIX advisory exclusive lock on `_ownership/<issue-key>/lock`, acquisition validates the entire chain, allocates `max(epoch)+1`, appends/fsyncs history, atomically replaces/fsyncs `current.json`, and then releases the lock. Missing/interior-corrupt history, epoch overflow, mismatched current/history, or unsupported locking is `CONFLICT` and cannot reset to epoch 1. A torn final line follows §7.6 recovery rules.
